...
class ReadTransactionsFromCsv(unittest.TestCase):
    @patch(
        ...
        read_data="transaction_id;amount;currency\n1;100.00;USD\n2;-50.00;EUR\n",
    )
    def test_read_transactions_from_csv(self, mock_file):
        ...
        mock_file.assert_called_once_with("/fake/path/to/file.csv", encoding="utf8")
...
class ReadTransactionsFromXlsx(unittest.TestCase):
    ...
    def test_read_transactions_from_xlsx(self, mock_read_excel):
        mock_df = pd.DataFrame({"TransactionID": ["T1", "T2"], "Amount": [100.0, -50.0], "Currency": ["USD", "EUR"]})
